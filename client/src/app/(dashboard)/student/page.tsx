...
export default function StudentDashboard() {
  ...
  // Process courses with progress information
  const coursesWithDetails = courses.map(course => {
    ...
    return { ...course, progress: prog, status };
  });
  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-6xl mx-auto pb-12">
      ...
      <div className="relative overflow-hidden bg-radial from-slate-900 to-zinc-950 p-6 md:p-8 rounded-2xl shadow-xl text-white border border-zinc-800">
      ...
      {pageLoading ? (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="h-28 bg-zinc-100 dark:bg-zinc-900 rounded-xl animate-pulse border border-zinc-200 dark:border-zinc-800" />
          ))}
        </div>
      ) : (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {/* Card 1: Total Courses */}
          <div className="bg-white dark:bg-zinc-900 p-5 rounded-2xl border border-zinc-150 dark:border-zinc-800/80 shadow-sm hover:shadow-md transition-all duration-300 group">
                ...
                <div className="mt-6 pt-5 border-t border-zinc-100 dark:border-zinc-800/60">
                  <div className="flex justify-between items-center mb-2 text-xs font-semibold">
                    <span className="text-zinc-400 flex items-center gap-1.5">
                      ...
                      <span className="truncate max-w-[130px]">{course.teacherName}</span>
                    </span>
                ...
                <Link
                  ...
                  className={`w-full py-2.5 px-4 rounded-xl text-center text-xs font-bold transition-all duration-200 flex items-center justify-center gap-2 ${
                    course.status === 'completed'
                      ...
                      : course.status === 'in-progress'
                        ? 'bg-blue-600 hover:bg-blue-700 text-white hover:shadow-md dark:bg-blue-600 dark:hover:bg-blue-700 shadow-sm'
